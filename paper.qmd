...
dem.syr.select <- dem_longMissing %>% filter(origin_iso3=="SYR", asylum_iso3 %in% c("SWE", "USA",  "MCO",  "LBN", "JOR", "GBR"))

p.imputations.syr.female <- ggplot(data = imputations.syr  ,
                            aes(y=reorder(asylum_country, femaleProp))) +
                    stat_histinterval(aes(x = femaleProp), slab_type  = "histogram")

p.imputations.syr.children <- ggplot(data = imputations.syr %>% filter(asylum_iso3 %in% c("SWE", "USA",  "MCO",  "LBN", "JOR", "GBR")),
